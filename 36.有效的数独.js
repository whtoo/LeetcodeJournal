    ...
    for(let i = 0; i < 9;i++){
        for(let j = 0; j < 9;j++){
            ...
            if(num !== '.'){
                ...
                rows[i].set(n,rows[i].has(n)?rows[i].get(n)+1:1)
                columns[j].set(n,columns[j].has(n)?columns[j].get(n)+1:1)
                boxes[boxIndex].set(n,boxes[boxIndex].has(n)?boxes[boxIndex].get(n)+1:1)
