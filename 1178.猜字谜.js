 ...
 * @param {string} s 
 */
var bithash = function(s){
    let res = 0
    const aCode = 'a'.charCodeAt(0)
    for(const c of s){
        res |= (1 << (c.charCodeAt(0) - aCode))
    }
    return res
}
/**
...
var findNumOfValidWords = function(words, puzzles) {
    ...
    for(const s of words){
        let hash = bithash(s)
        map.set(hash,map.has(hash)?map.get(hash)+1:1)
    }
    for(const s of puzzles){
        let hash = bithash(s)
        let sum = 0
        for(let i = hash;i > 0;i = (i-1) & hash){
            if(((1 << (s.charCodeAt(0) - acode))&i) != 0){
                sum += (map.has(i)?map.get(i):0)
            }
        }
        list.push(sum)
    }
