...
var maskPII = function(S) {
    ...
    // if(atIdx !== -1){
    //     let name = S.slice(0,atIdx)
    //     return name[0] + "*****" + name[name.length-1] + S.slice(atIdx)
    // } else {
    //     let nums = []
    //     for(const ch of S){
    //         if(!isNaN(ch) && ch !== " "){
    ...
    //         }
    //     }
    //     let prefix = (nums.length > 10)?"+"+"*".repeat(nums.length - 10)+"-":""
    //     return prefix+"***-***-"+nums.slice(nums.length-4).join('')
    // }
    S = S.replace(/[\+\-\(\)\s]/g,'')
    return S.includes('@')
    ? S.toLowerCase().replace(/^(\w)[\w]*(\w)(@.*)$/, '$1*****$2$3')
    : S.length > 10
    ? S.replace(/.*([\d]{4})$/, `+${'*'.repeat(S.length - 10)}-***-***-$1`)
    : S.replace(/.*([\d]{4})$/,'***-***-$1')
};
// @lc code=end
